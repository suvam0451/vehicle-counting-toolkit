...
struct VehicleCountGroup
{
    ...
    VehicleCountGroup() : motorcycleCount(0), carcount(0), truckCount(0), busCount(0){};
};
...
struct TextElement
{
    ...
    TextElement(string str, double val) : text(str), color(118, 185, 0), fontScale(val){};
};

// function to display HUD and stats
void showHUD(Mat &target, VehicleCountGroup &values)
{
    string motorcycleText = "Number of motorcycles: " + to_string(values.motorcycleCount);
    ...
    putText(target, motorcycleText, Point(10, target.rows / 2 + font_height * stringIndex++),
            cv::FONT_HERSHEY_DUPLEX,
            0.75,
            CV_RGB(118, 185, 0), FONT_WEIGHT);
    putText(target, carText, Point(10, target.rows / 2 + font_height * stringIndex++),
            cv::FONT_HERSHEY_DUPLEX,
            0.75,
            CV_RGB(118, 185, 0), FONT_WEIGHT);
    putText(target, truckText, Point(10, target.rows / 2 + font_height * stringIndex++),
            cv::FONT_HERSHEY_DUPLEX,
            0.75,
            CV_RGB(118, 185, 0), FONT_WEIGHT);
    putText(target, busText, Point(10, target.rows / 2 + font_height * stringIndex++),
            cv::FONT_HERSHEY_DUPLEX,
            0.75,
            CV_RGB(118, 185, 0), FONT_WEIGHT);
...
void showHelpText(Mat &target, int values)
{
    ...
    for (auto it : msgs)
    {
        putText(target, it.text, basePosition + Point(0, font_height * stringIndex++),
                cv::FONT_HERSHEY_DUPLEX, fontScale, it.color, FONT_WEIGHT);
    }
}

void drawLine(Mat &target, float start_x, float start_y, float end_x, float end_y)
{
...
int testDetectionValidity()
{
    ...
    {
        drawRectangle(image, objs["center_x"], objs["center_y"], objs["class_id"]);
    }
